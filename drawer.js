...
function renderArea(area, players, focus, old) {
  ...
  renderPlayers(area, players, focus, world.pos);
  ...
  for (var i in players) {
    ...
    if(player.flashlight_active){ //LIGHTTIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIING
      ...
      else if(player.joystickActive){
        var angle = Math.atan2(gamepad.axes[1], gamepad.axes[0]);
        angle = (angle * 180) / Math.PI;
        player.inputAng = angle; 
      }
      else if(player.mouseActive){
  ...
  if (toggleUI){ renderUI(area, players, focus) }
  if (toggleMinimap) { renderMinimap(area, players, focus) }
  ...
  if(settings.dev){
    context.fillStyle = "gray";
    context.strokeStyle = 'gray';
    const safePoint = (player.safePoint) ? "Safe Point: {X:" + Math.round(player.safePoint.pos.x*fov) + ", Y: " + Math.round(player.safePoint.pos.y*fov) + "} ([), to clear (])" : "Press ([) to make a safe point"; 
          ...
          if(!entities[i][j].texture)context.arc(width / 2 + (area.pos.x + entities[i][j].pos.x - focus.x) * fov, height / 2 + (area.pos.y + entities[i][j].pos.y - focus.y) * fov, entities[i][j].radius * fov, 0, Math.PI * 2, true);
            ...
            switch(entities[i][j].texture){
              ...
              case "sweet_tooth_item": 
                Texture = sweet_tooth_item;
                break;
              case "vengeance_projectile": 
                Texture = vengeance_projectile;
                break;
            }
            if(Texture){
  ...
  for (var i in players) {
    ...
    if(player.aura){
      if(player.auraType == 0){
        ...
        context.beginPath();
        context.fillStyle = "rgba(0, 178, 112, 0.2)";
        context.arc(width / 2 + (player.pos.x - focus.x) * fov, height / 2 + (player.pos.y - focus.y) * fov, 165/32 * fov, 0, Math.PI * 2, true);
        context.fill();
        context.closePath();
      }
    }
    ...
    if (!player.die) {
      ...
      let timeLeft = Math.round(this.game.worlds[this.game.players[0].world].areas[this.game.players[0].area].death_timer - player.timeDead / 1000);
      if (player.className == "Chrono"){
        timeLeft = Math.round(Number(document.getElementById("death_timer").value) - player.timeDead / 1000)
      }
      ...
      context.lineWidth = 1;
      //context.fillText(lol, player.pos.x - focus.x, player.pos.y - focus.y);
      //context.strokeText(timeLeft, player.pos.x + worldPos.x, player.pos.y + worldPos.y);
      context.strokeText(timeLeft, 640, 370);
    }
    if (player.type==7) {
      if (player.shape>0) {
      ...
      if(!player.reaperShade)if(player.mortarTime<1000 || player.className != "Cent")context.arc(width / 2 + (player.pos.x - focus.x) * fov, height / 2 + (player.pos.y - focus.y) * fov, player.radius * fov, 0, Math.PI * 2, true);
    }
    context.fill();
    context.closePath();
...
function renderUI(area, players, focus) {
  ...
  if(game.players[0].hasAB){
    ...
    if (players[0].className == "Necro"){
      players[0].firstAbilityCooldown = (players[0].resurrectAvailable) ? players[0].pelletTotal : players[0].pelletTotal - players[0].pelletCount - 0.000001;
      players[0].firstTotalCooldown = players[0].pelletTotal;
    }
    for(var a = 0; a<2; a++){
