...
class World {
  constructor(pos, id, map, isImported = false) {
    this.pos = pos;
    this.areas = [];
    this.id = id;
    this.name = "Default";
  ...
  fromJson(json) {
    ...
    if (properties) {
      ...
      if (properties.death_timer !== undefined) {
        this.death_timer = properties.death_timer;
      }
    ...
    for (var i = 0; i < areas.length; i++) {
      ...
      area.death_timer = this.death_timer;
      ...
      if (propertiesC) {
        ...
        if (propertiesC.pellet_multiplier !== undefined) {
          area.pellet_multiplier = propertiesC.pellet_multiplier;
        }
        if (propertiesC.death_timer !== undefined) {
          area.death_timer = propertiesC.death_timer;
        }
      }
      for (var j = 0; j < zones.length; j++) {
        ...
        for (var k in spawner) {
          ...
          var object = {
            ...
            speed: (this.isImported) ? values.speed / 30 : (settings.legacy_speed ? values.speed : values.speed / 30),
...
class Area {
  ...
  update(time, players, worldPos) {
    ...
    for (var i in this.entities) {
      for (var j in this.entities[i]) {
        if (players[0].className == "Chrono" && this.entities[i][j].isEnemy){
          this.entities[i][j].chrono_exists = true;
        }
  ...
  load() {
    ...
    for (var i = 0; i < pelletsAtZone; i++) {
      ...
      var pellet = new Pellet(new Vector(posX, posY), this.pellet_multiplier)
